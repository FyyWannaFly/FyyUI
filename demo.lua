...
local function notify(title, content, kind)
	menu:Notify({ Title = title, Content = content, Type = kind or "Info", Duration = 3 })
end

-- Authorization is completed by the public loader before this protected demo
-- is downloaded. This runtime owns only the handed-off heartbeat lifecycle.
...
local function requestJson(path, method, body)
	...
	local options = { Url = API_ORIGIN .. path, Method = method, Headers = { ["Accept"] = "application/json", ["Cache-Control"] = "no-store" } }
	...
	if status < 200 or status >= 300 or decoded.status ~= "ok" then return nil, tostring(decoded.code or decoded.reasonCode or "REQUEST_REJECTED"), decoded end
	return decoded, nil
end
...
local function stopAccessHeartbeat()
	...
	if session then
		accessSequence += 1
		task.spawn(function()
			pcall(requestJson, "/api/v1/check/heartbeat", "POST", { sessionId = session.sessionId, sessionToken = session.sessionToken, sequence = accessSequence, clientState = "stopping" })
...
local function startAccessHeartbeat(session)
	...
	task.spawn(function()
		while accessSession == session and accessHeartbeatToken == token do
			...
			accessSequence += 1
			local heartbeat, heartbeatError = requestJson("/api/v1/check/heartbeat", "POST", { sessionId = session.sessionId, sessionToken = session.sessionToken, sequence = accessSequence, clientState = "running" })
			if not heartbeat or heartbeat.state ~= "active" then
				accessSession = nil
				notify("Access Ended", tostring(heartbeat and heartbeat.reasonCode or heartbeatError or "SESSION_ENDED"), "Error")
				break
			end
...
local handoff = environment().__FYY_ACCESS_HANDOFF
environment().__FYY_ACCESS_HANDOFF = nil
local preauthorizedSession = type(handoff) == "table" and type(handoff.session) == "table" and handoff.session or nil
if preauthorizedSession then
	startAccessHeartbeat(preauthorizedSession)
	notify("Access Granted", "Authorized loader session active.", "Success")
else
	notify("Loader Session Missing", "Run this demo through the Fyy Community loader.", "Warning")
end
handoff, preauthorizedSession = nil, nil

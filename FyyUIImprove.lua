...
return (function()
	...
	function Menu:_applyResponsiveLayout()
		...
		if self._layoutTopbarInfo then
			self._layoutTopbarInfo(needsCompact or self.Frame.Size.X.Offset < 520)
		end
	...
	function Menu.new(options, theme)
		...
		assert(
			options.TopbarInfo == nil or options.TopbarInfo == false or type(options.TopbarInfo) == "table",
			"FyyUI Menu: TopbarInfo must be false or a table"
		)
		local topbarInfoOptions = type(options.TopbarInfo) == "table" and options.TopbarInfo or {}
		for _, key in ipairs({ "Game", "Access" }) do
			assert(
				topbarInfoOptions[key] == nil or type(topbarInfoOptions[key]) == "string",
				("FyyUI Menu: TopbarInfo.%s must be a string"):format(key)
			)
		end
		local requestedAccess = string.upper(topbarInfoOptions.Access or "TRIAL")
		assert(requestedAccess == "TRIAL" or requestedAccess == "PREMIUM", "FyyUI Menu: TopbarInfo.Access must be Trial or Premium")
		...
		self.TopbarInfoConfig = {
			Enabled = options.TopbarInfo ~= false,
			Game = topbarInfoOptions.Game or tostring(game.Name or "Game"),
		...
		if self.TopbarInfoConfig.Enabled then
			...
			self._layoutTopbarInfo(size.X < 520)
		end
		...
		self.Title = U.Create("TextLabel", {
			Name = "Title",
			Size = UDim2.new(1, -(titleX + titleRightInset), 1, 0),
			Position = UDim2.fromOffset(titleX, 0),
			BackgroundTransparency = 1,
			Text = "",
		...
		self._refreshTitle = function()
			...
			if branded then
				...
				self.Title.Size = UDim2.new(1, -(titleX + 30 + titleGap + titleRightInset), 1, 0)
			else
				self.Title.Text = self._titleText
				self.Title.Position = UDim2.fromOffset(titleX, 0)
				self.Title.Size = UDim2.new(1, -(titleX + titleRightInset), 1, 0)
			end
		end
		self._refreshTitle()
		...
		self.SidebarList = U.Create("ScrollingFrame", {
			Name = "TabList",
			Size = UDim2.new(1, 0, 1, -supportReserve),
		...
		-- Overview intentionally remains unmounted in the improved layout.
		-- Its implementation is retained for compatibility, but no tab is created.
	...
	function Menu:_positionActiveBar(tab, animate)
		...
		local barH = 20
		local sbH = self.SidebarList and self.SidebarList.AbsoluteSize.Y or 0
		local fade = 1
		if sbH > 0 then
			if y < 8 then
				fade = math.max(y / 8, 0)
	...
	function Menu:_setInternalsVisible(visible)
		...
		if self.TopbarInfoHolder then
			self.TopbarInfoHolder.Visible = visible
		end
		if self.Topbar then
			for _, child in ipairs(self.Topbar:GetChildren()) do
				if child:IsA("ImageButton") then
	...
	function Menu:SetGameName(name)
		if self._destroyed or not self.GameInfoLabel then
			return false, "topbar info unavailable"
		end
		if type(name) ~= "string" or name == "" then
			return false, "expected non-empty string"
		end
		self.TopbarInfoConfig.Game = name
		self.GameInfoLabel.Text = string.upper(name)
		return true
	end

	function Menu:SetAccessStatus(status)
		if self._destroyed or not self.AccessInfoLabel then
			return false, "topbar info unavailable"
		end
		if type(status) ~= "string" then
			return false, "expected Trial or Premium"
		end
		status = string.upper(status)
		if status ~= "TRIAL" and status ~= "PREMIUM" then
			return false, "expected Trial or Premium"
		end
		self.TopbarInfoConfig.Access = status
		self._refreshAccessCard()
		return true
	end

...
return function(FyyUI)
	assert(FyyUI.Version == "0.15.0", "library version must identify the dropdown, minimize, and notification release")
	...
	assert(legacyConfig.Schema == "FyyUI.Config.v1" and legacyConfig.Version == "0.15.0", "zero-argument config export must retain the v1 contract")
	...
	assert(single:SetValue("One", true) and single:GetValue() == "", "AllowNone single dropdown must unselect the active option")
	...
	collapsible:Divider()
	assert(menu:SetTheme("Light") and menu:SetTheme("Amoled") and menu:SetTheme("Dark"), "theme switching must update lightweight and controller components without receiver mismatch")
	...
	assert(v2.Schema == "FyyUI.Config.v2" and v2.SchemaVersion == 2 and v2.Version == "0.15.0", "explicit v2 export must use the versioned JSON-safe envelope")
